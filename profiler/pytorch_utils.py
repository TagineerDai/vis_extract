...
# global var for flops calc
count_ops = 0
count_params = 0

def model_check(current_model, depth=0):
    ...
    for module_name in current_model._modules:
        ...
        if not parameter_list:
            pytorch_model_check(current_module, depth+1)
        else:
            for param_name in parameter_list:
                try:
                    print(current_depth + '|-' + param_name, 'requries_grad is', \
                          str(current_module._parameters[param_name].requires_grad)+'.')
                except:
...
def measure_layer(layer, x):
    ...
    ### unknown layer type
        ...
        raise TypeError('pytorch_utils.py: unknown layer type: %s' % type_name)

    count_ops += delta_ops
    count_params += delta_params
    return

def summarize_model(model, input):
    global count_ops, count_params
    count_ops = 0
    count_params = 0
...
def calculate_flops(model, input):
    """
    returned float ops of model.
    print(flops / 1e9, 'Gflops')
    """
    # input should in size of(1, 3, H, W) with batch_size exactly being 1.
    # e.g. input=next(iter(handler.generator.DataLoader))[0]
    count_ops, count_params = summarize_model(model, input)
    flops = count_ops
    return flops

def calculate_size(model):
    """
    returned parameter size unit: byte
    print(model_size / 1e6, 'MB')
    """
    # input should in size of(1, 3, H, W) with batch_size exactly being 1.
    # e.g. input=next(iter(handler.generator.DataLoader))[0]
    count_ops, count_params = summarize_model(model, input)
